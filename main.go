...
import (
	...
	"github.com/DominicArgento/CSCE4600P2/builtins"
)
...
func handleInput(w io.Writer, input string, exit chan<- struct{}) error {
	...
	case "exit":
		...
		return nil
	case "echo":
		// Call the function to handle the echo command
		return builtins.Echo(w, args...)
	case "pwd":
		// Call the function to handle the pwd command
		return builtins.PrintWorkingDirectory(w, args...)
	case "mkdir":
		// Call the function to handle the mkdir command
		return builtins.MakeDirectory(args...)
	case "rmdir":
		// Call the function to handle the rmdir command
		return builtins.RemoveDirectory(args...)
	case "touch":
		// Call the function to handle the touch command
